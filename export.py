...
2020-08-11 10:07:39
                    1. 导出avi格式，查询语句需要按start_time排序
                    2. 导出gif格式，查询语句需要按start_time排序
2020-08-20 10:24:03
                    1. 导出报告，新增report文件夹
...
def export_gif(category_abbr_en,starttime,endtime,frame_ps,output_fullpath):
    ...
    sqlcmd='SELECT data_file_info.path,data_file_info.filename FROM data_file_info WHERE data_file_info.category_abbr_en = %s and start_time BETWEEN %s and %s order by start_time'%(category_abbr_en,search_starttime,search_endtime)
...
def export_avi(category_abbr_en,starttime,endtime,frame_ps,output_fullpath):
    ...
    category_abbr_en='\'%s\''%(category_abbr_en)   
    #sqlcmd='SELECT data_file_info.path,data_file_info.filename FROM data_file_info INNER JOIN data_category ON data_file_info.category_id = data_category.category_id WHERE data_category.category_abbr_en = %s and start_time BETWEEN %s and %s'%(category_abbr_en,search_starttime,search_endtime)
    sqlcmd='SELECT data_file_info.path,data_file_info.filename FROM data_file_info WHERE category_abbr_en = %s and start_time BETWEEN %s and %s order by start_time'%(category_abbr_en,search_starttime,search_endtime)
...
def export_partial_data(category_abbr_en,starttime,endtime,output_fullpath):
    ...
    sqlcmd="SELECT data_file_info.path, data_file_info.filename FROM data_file_info WHERE category_abbr_en = '%s' and data_file_info.start_time BETWEEN %s and %s order by start_time"%(category_abbr_en,search_starttime,search_endtime)
